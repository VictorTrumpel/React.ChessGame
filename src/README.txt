FSD не может зависить от DEPRECATED,
но DEPRECATED может зависить от FSD


Проект на FSD состоит из слоев (layers), каждый слой состоит из слайсов (slices) 
и каждый слайс состоит из сегментов (segments).
...
Критерий: Это не относится к БЛ и является общим переиспользуемым кодом?
2. entities (сущности) — бизнес-сущности.(например, User, Product, Order)
Критерий: Это относится непосредственно к бизнес сущности?
...
Критерий: Это относится к действию пользователя, представляющему бизнес-ценность?
...
Критерий: Это самомстоятельный полноценный блок страницы с конткретными действиями?
5. pages (страницы) — композиционный слой для сборки полноценных страниц из сущностей, фич и виджетов.
Критерий: Это относится к конкретной странице/экрану? 
6. processes — сложные сценарии, покрывающие несколько страниц.(например, авторизация)
Критерий: Это относится к конкретному юзкейсу протекающему через несколько страниц
7. app — настройки, стили и провайдеры для всего приложения.
Критерий: Это общая инициализирующая логика приложения?

СЛАЙСЫ(SLICES)
Затем есть слайсы, разделяющие код по предметной области. Они группируют логически связанные модули,
что облегчает навигацию по кодовой базе. Слайсы не могут использовать другие слайсы на том же слое,
что обеспечивает высокий уровень связности (cohesion) при низком уровне зацепления (coupling).

СЕГМЕНТЫ(SEGMENTS)
В свою очередь, каждый слайс состоит из сегментов. Это маленькие модули, главная задача которых 
— разделить код внутри слайса по техническому назначению. Самые распространенные сегменты — ui, model (store, actions),
api и lib (utils/hooks), но в вашем слайсе может не быть каких-то сегментов, могут быть другие, по вашему усмотрению.

NOTE
1. В большинстве случаев рекомендуется располагать api и config только в shared-слое
